...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.store('designer', {
        ...
        init() {
            ...
            setTimeout(() => {
                if (this.isLoading) {
                    debugWarn('5 second timeout reached, forcing loading overlay to hide');
                    this.hideLoading();
                }
            }, 5000);
        ...
        setLoadingState(type, loaded) {
            ...
            if (this.loadingStates.model && this.loadingStates.design && this.loadingStates.fonts) {
        ...
        applyBumpmapToLayer(layer, materialKey) {
            ...
            textureLoader.load(
                ...
                (texture) => {
                    ...
                    // Apply texture scale
                    const scale = bumpmap.scale || 1;
                    texture.repeat.set(scale, scale);

                    // Apply to material
                    layer.mesh.material.bumpMap = texture;
                    layer.mesh.material.bumpScale = bumpmap.bumpScale || 1;
                    ...
                    layer.material = materialKey;
                    layer.roughness = bumpmap.roughness || 1;
                    layer.metalness = bumpmap.metalness || 0;

                    debugLog(`Applied bumpmap to layer ${layer.name}:`, materialKey);
                },
                undefined,
                (error) => {
        ...
        setupThreeJS() {
            ...
            // Mouse events
            renderer.domElement.addEventListener('click', (event) => this.onCanvasClick(event));
        ...
        async loadModel() {
            ...
            try {
                ...
                // Position camera (matching vanilla designer.js fitCameraToModel function exactly)
                ...
                // Set zoom limits based on model size (matching vanilla designer.js)
        ...
        initializeLayers(model) {
            ...
            model.traverse((child) => {
                ...
                if (child.isMesh) {
                    meshCount++;
                    debugLog(`Found mesh #${meshCount}:`, child.name, 'Type:', child.type);
                    ...
                    if (!child.name.includes("polySurface")) {
                        ...
                        if (hasProduct && meshSettings[child.name]) {
                            ...
                            if (settings.initialColor) {
                                child.material.color.setHex(parseInt(settings.initialColor, 16));
                            }

                            // Apply initial bumpmap if specified and allowed
                            if (settings.canChangeBumpmap && this.product?.initialBumpmap && this.product.initialBumpmap !== 'none') {
                                this.applyBumpmapToLayer(this.layers[child.name], this.product.initialBumpmap);
                                debugLog(`Applied initial bumpmap '${this.product.initialBumpmap}' to layer ${child.name}`);
                            }
            ...
            debugLog(`Traverse complete. Examined ${childCount} children, found ${meshCount} meshes`);

            // Set initial layer
            const initialLayer = this.product?.initialLayer || Object.keys(this.layers)[0];
        ...
        onCanvasClick(event) {
            ...
            const meshes = Object.values(this.layers).map(layer => layer.mesh).filter(Boolean);
        ...
        rotateCamera(direction) {
            ...
            if (direction === 'left') {
                currentPosition.applyAxisAngle(new THREE.Vector3(0, 1, 0), rotationAmount);
            } else {
                currentPosition.applyAxisAngle(new THREE.Vector3(0, 1, 0), -rotationAmount);
            }
...
window.rotateView = (direction) => Alpine.store('designer').rotateView(direction);
window.rotateCamera = (direction) => Alpine.store('designer').rotateCamera(direction);
window.zoomCamera = (direction) => Alpine.store('designer').zoomCamera(direction);
...
window.editDesignName = () => {
    const newName = prompt('Enter design name:', Alpine.store('designer').designName);
    if (newName && newName.trim()) {
        Alpine.store('designer').designName = newName.trim();
    }
};
window.setCurrentLayerColor = (color) => Alpine.store('designer').setLayerColor(color);
window.setCurrentLayerColorFromHex = (hex) => Alpine.store('designer').setLayerColor('#' + hex);
window.addImageDecal = () => Alpine.store('ui').openImageBank();
window.closeDropdownAndAddText = () => Alpine.store('designer').addDecal('text', { text: 'Sample Text', font: 'Roboto', color: '000000' });
window.closeDropdownAndAddFade = () => Alpine.store('designer').addDecal('fade', {});
