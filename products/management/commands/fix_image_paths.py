...
class Command(BaseCommand):
    help = 'Fix product image paths from assets/ to static/'

    def handle(self, *args, **options):
        products = Product.objects.all()
        
        for product in products:
            if product.thumbnail and product.thumbnail.startswith('assets/'):
                old_path = product.thumbnail
                new_path = product.thumbnail.replace('assets/', '', 1)
